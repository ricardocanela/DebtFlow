...
resource "aws_eip" "nat" {
  domain = "vpc"
  tags   = { Name = "debtflow-${var.environment}-nat-eip" }
}

resource "aws_nat_gateway" "main" {
  allocation_id = aws_eip.nat.id
  subnet_id     = aws_subnet.public[0].id
  tags          = { Name = "debtflow-${var.environment}-nat" }
}
...
resource "aws_route_table" "public" {
  ...
  tags = { Name = "debtflow-${var.environment}-public-rt" }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "debtflow-${var.environment}-private-rt" }
}
...
resource "aws_security_group" "db" {
  ...
  tags = { Name = "debtflow-${var.environment}-db-sg" }
}
...
resource "aws_security_group" "cache" {
  ...
  tags = { Name = "debtflow-${var.environment}-cache-sg" }
}
